...
/** Verifies Cluster Sharding for bucket entities in a single-node cluster (design
  * D29): commands route to one entity, the persistence id stays `bucket|<name>`, and
  * generation counters work end-to-end through the real journal.
  */
final class BucketShardingIT
  ...
  override def afterStart(): Unit =
    ...
    )(c => Using.resource(c.createStatement())(st => { val _ = st.execute(sql) }))
