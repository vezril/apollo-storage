...
/** Cluster Sharding for bucket entities (design D29): each bucket is a cluster-wide
  * singleton, addressed by `EntityRef`. The entity type key name is `bucket`, so the
  * sharded persistence id is `bucket|<name>` — the frozen contract (design D2) is
  * preserved without any change to `BucketEntity`.
  */
