...
client-supplied value is trusted or echoed), the ID rides in the SLF4J MDC (promoted to a top-level
JSON field by the structured-logging encoder), and log verbosity is env-configurable so Apollo's own
code can be raised to TRACE without third-party noise.
