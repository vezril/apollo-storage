...
Multipart upload provides the atomic staging the filesystem backend gets from temp-file→rename, and the
existing blob-gc reconciliation runs against S3 listings. Selected at runtime via `BLOB_BACKEND=s3`
(see the blob-storage capability). Realized on the JVM with Alpakka S3 (`pekko-connectors-s3`).
